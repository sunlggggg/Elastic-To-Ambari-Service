...
class ESSlave(Script):
    ...
    def install(self, env):
        ...
        #Use ES built-in JDK
        fp = file(format("{es_slave_bin}/elasticsearch-env"))
        lines = []
        for line in fp:
          lines.append(line)
        fp.close()
        ...
        s=''.join(lines)
        fp = file(format("{es_slave_bin}/elasticsearch-env"), 'w')
        fp.write(s)
        fp.close()
    ...
    def configure(self, env):
        ...
        File(format("{es_slave_conf_dir}/jvm.options"),
        ...
        File(format("{es_slave_conf_dir}/log4j2.properties"),
