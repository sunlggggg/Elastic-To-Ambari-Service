...
class ESMaster(Script):
    ...
    def install(self, env):
        ...
        #Use ES built-in JDK
        fp = file(format("{es_master_bin}/elasticsearch-env"))
        lines = []
        for line in fp:
          lines.append(line)
        fp.close()
        ...
        s=''.join(lines)
        fp = file(format("{es_master_bin}/elasticsearch-env"), 'w')
        fp.write(s)
        fp.close()
    ...
    def configure(self, env):
        ...
        log4j2_configurations = params.config['configurations']['elasticsearch-log4j2']
        File(format("{es_master_conf_dir}/log4j2.properties"),
             content=Template("elasticsearch.log4j2.options.j2", configurations=log4j2_configurations),
             owner=params.es_user,
             group=params.es_group
             )
