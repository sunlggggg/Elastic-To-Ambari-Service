...
# kibana config
server_host = str(config['configurations']['kibana-config']['server.host'])
...
elasticsearch_hosts = str(config['configurations']['kibana-config']['elasticsearch.hosts'])
# Need to parse the comma separated hostnames to create the proper string format within the configuration file
# Expects the format ["host1:port","host2:port"]
elasticsearch_hosts_list=elasticsearch_hosts.split(',')
elasticsearch_hosts='[' +  ','.join('"' + x + '"' for x in elasticsearch_hosts_list) + ']'
...
i18n_locale = str(config['configurations']['kibana-config']['i18n.locale'])
